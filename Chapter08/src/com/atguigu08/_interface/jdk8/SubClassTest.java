...
public class SubClassTest {
    public static void main(String[] args) {
        //知识点一：接口中声明的静态方法只能通过接口来调用，不能使用实现类（Subclass）进行调用
        CompareA.method1();
        ...
        ///知识点3:类实现了两个接口，而两个接口中定义了同名同参数的跳认方法。则实现类在没有重写此两个接口默认方法的情况下、会报错。---->接口冲突
        //要求:此时实现类必须要重写接口中定义的同名同参数的方法.|
        subClass.method3();

        //知识点4:子类（或实现类）继承了父类并实现了接口。父类和接口中声明了网名同参数的方法。(其中，接口中的方法是默认方法)。
